...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 rd = normalize(vec3(p,1.3)); // 1.3 is focal length
    vec2 mo = iMouse.xy / iResolution.xy-.5; // Mouse position in normalized coordinates
    mo = (mo==vec2(-.5))?mo=vec2(-0.1,0.1):mo;   
	mo.x *= iResolution.x/iResolution.y;  // Set aspect ratio
    rd.yz *= mm2(mo.y); // Tilt camera
