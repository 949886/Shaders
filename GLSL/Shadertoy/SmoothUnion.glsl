// Uses my raymarcher template.

// Precision declaration and uniforms:
...
// Camera variables:
vec3 cameraPos = vec3(0.0, 0.0, 1000.0);
...
Surface SmoothUnion(in Surface surface1, in Surface surface2, in float smoothness) {
    ...
    vec3 ambientColor = mix(surface2.ambientColor, surface1.ambientColor, interpolation);// - smoothness * interpolation * (1.0 - interpolation);
    vec3 diffuseColor = mix(surface2.diffuseColor, surface1.diffuseColor, interpolation);// - smoothness * interpolation * (1.0 - interpolation);
    vec3 specularColor = mix(surface2.specularColor, surface1.specularColor, interpolation);// - smoothness * interpolation * (1.0 - interpolation);
    float shininess = mix(surface2.shininess, surface1.shininess, interpolation);// - smoothness * interpolation * (1.0 - interpolation);
    float signedDistance = mix(surface2.signedDistance, surface1.signedDistance, interpolation) - smoothness * interpolation * (1.0 - interpolation);
    return Surface(ambientColor,
                   diffuseColor,
                   specularColor,
                   shininess,
                   signedDistance);
...
Surface mapScene(in vec3 p) {
    vec4 metashapeInfo1 = vec4(vec2(sin(iTime * 0.75), cos(iTime * 1.0)) * 200.0, 0.0, 120.0);
    vec4 metashapeInfo2 = vec4(vec2(cos(iTime * 1.0), cos(iTime * 1.25)) * 200.0, 0.0, 140.0);
    vec4 metashapeInfo3 = vec4(vec2(sin(iTime * 1.25), cos(iTime * 0.5)) * 200.0, 0.0, 160.0);

    Surface metashape1 = Surface(vec3(0.0, 0.0, 0.0),
                                 vec3(1.0, 0.0, 0.0),
                                 vec3(1.0, 1.0, 1.0), 8.0,
                                 sdCuboid(Rotate(Translate(p, metashapeInfo1.xyz), vec3(0.0, 0.0, iTime * 40.0)), 2.0 * metashapeInfo1.w, 2.0 * metashapeInfo1.w, 2.0 * metashapeInfo1.w));

    Surface metashape2 = Surface(vec3(0.0, 0.0, 0.0),
                                 vec3(0.0, 1.0, 0.0),
                                 vec3(1.0, 1.0, 1.0), 64.0,
                                 sdCylinder(Rotate(Translate(p, metashapeInfo2.xyz), vec3(0.0, 0.0, iTime * -20.0)), 2.0 * metashapeInfo2.w, metashapeInfo2.w));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 rayOrigin = cameraPos + cameraForward * cameraFocalLength;
    vec3 rayDirection = normalize(rayOrigin - (cameraPos - cameraRight * xy.x - cameraUp * xy.y));
